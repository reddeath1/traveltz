           ...
           $data = $this->get_av_result($this->encode($params));
            ...
            if(count($data) > 0){
                ...
                foreach ($data as $item) {
                    ...
                    $companies = $this->company($bid,$from,$to);
            ...
            print_r((!empty($result) ? $this->encode(array('data'=>$result,'count'=>count($data),'results'=>$results,'default_company'=>$companies)) : ''));
       }else{
           $this->message = "All fields are required";
       }
   }

    public function company($id,$r1,$r2)
    {
        $cp = $this->get_company(); // TODO: Change the autogenerated stub
        ...
        if(count($cp) > 0) {
            foreach ($cp as $company) {
                ...
                $count_companies = $this->count_companies($id,$r1,$r2);

                $companies_results .= " <span class=\"badge badge-secondary\">
								<div class=\"form-check form-check-inline\">
									<input class=\"form-check-input\" type=\"checkbox\" class='company $cp_id' id=\"inlineCheckbox1\" value=\"option1\">
